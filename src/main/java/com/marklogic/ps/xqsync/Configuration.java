...
public class Configuration extends AbstractConfiguration {
    ...
    /* fields */
    protected final Collection<ContentPermission> permissionRoles = new ArrayList<>();
    ...
    public void configure() throws Exception {
        ...
        String placeKeysString = properties.getProperty(OUTPUT_FORESTS_KEY);
        if (placeKeysString != null) {
            placeKeys = placeKeysString.split(CSV_SCSV_SSV_REGEX);
            logger.info("placeKeys = " + placeKeysString);
        }

        String outputFormatFilterString = properties.getProperty(OUTPUT_FILTER_FORMATS_KEY);
        if (null != outputFormatFilterString) {
            outputFormatFilterString = outputFormatFilterString.trim();
            if (null != outputFormatFilterString && outputFormatFilterString.length() > 1) {
                outputFormatFilters = outputFormatFilterString.split(CSV_SCSV_SSV_REGEX);
                logger.finest(this + " outputFormatFilters = " + Utilities.join(outputFormatFilters, ","));
            }
        }
        ...
        if (null != outputCollectionsString) {
            ...
            if (null != outputCollectionsString && outputCollectionsString.length() > 1) {
                outputCollections = outputCollectionsString.split(CSV_SCSV_SSV_REGEX);
    ...
    private void configureInput() throws IOException, URISyntaxException, XccException, SyncException, KeyManagementException, NoSuchAlgorithmException {
        ...
        if (inputPath != null) {
            ...
            String inputConnectionString = properties.getProperty(INPUT_CONNECTION_STRING_KEY);
            if (inputConnectionString == null) {
                throw new IOException("missing required property: " + INPUT_CONNECTION_STRING_KEY);
            }
    ...
    protected void configureOutput() throws Exception {
        ...
        if (null != outputPackagePath) {
            ...
            if (null != outputPath) {
                ...
                String outputConnectionString = properties.getProperty(OUTPUT_CONNECTION_STRING_KEY);
                if (outputConnectionString == null) {
                    throw new IOException("missing required property: " + OUTPUT_CONNECTION_STRING_KEY);
                }
                ...
                for (int i = 0; i < outputConnection.length; i++) {
                    outputConnection[i] = new Connection(new URI(outputConnectionStrings[i]));
    ...
    */
    void configureThrottling() {
        throttledEventsPerSecond = Double.parseDouble(properties.getProperty(THROTTLE_EVENTS_KEY));
        throttledBytesPerSecond = Integer.parseInt(properties.getProperty(THROTTLE_BYTES_KEY));
    }
     ...
     *
     */
    private void configureTimestamp(String timestampString)
            throws RequestException {
        if (null != timestampString) {
            ...
            if (null == sess) {
                ...
                if (Configuration.INPUT_TIMESTAMP_AUTO.equals(timestampString)) {
    ...
    protected boolean isValidConnectionString(String connectionString) {
        return connectionString.startsWith(XCC_PREFIX)
                || connectionString.startsWith(XCCS_PREFIX)
                || connectionString.startsWith(XCC_PREFIX_OLD);
    }
    ...
    public boolean isCopyCollections() {
        return Utilities.stringToBoolean(properties.getProperty(COPY_COLLECTIONS_KEY));
    }
    ...
    public boolean isCopyPermissions() {
        return Utilities.stringToBoolean(properties.getProperty(COPY_PERMISSIONS_KEY));
    }
    ...
    public boolean isCopyProperties() {
        return Utilities.stringToBoolean(properties.getProperty(COPY_PROPERTIES_KEY));
    }
    ...
    public boolean isCopyQuality() {
        return Utilities.stringToBoolean(properties.getProperty(COPY_QUALITY_KEY));
    }
    ...
    public boolean isFatalErrors() {
        return Utilities.stringToBoolean(properties.getProperty(FATAL_ERRORS_KEY));
    }
    ...
    public boolean isRepairMultipleDocumentsPerUri() {
        return Utilities.stringToBoolean(properties.getProperty(REPAIR_MULTIPLE_DOCUMENTS_PER_URI_KEY));
    }
    ...
    public boolean isRepairInputXml() {
        return Utilities.stringToBoolean(properties.getProperty(REPAIR_INPUT_XML_KEY));
    }
    ...
    public boolean isDeleteOutputCollection() {
        return Utilities.stringToBoolean(properties.getProperty(DELETE_COLLECTION_KEY, "false"));
    }
    ...
    public Session newInputSession() {
        logger.fine(null == inputConnection ? null : inputConnection.toString());
    ...
    public String getConfigurationClassName() {
        ...
        return properties.getProperty(CONFIGURATION_CLASSNAME_KEY, CONFIGURATION_CLASSNAME_DEFAULT);
    }
    ...
    private String[] getDelimitedPropertyValues(String key) {
        return getDelimitedPropertyValues(key, "\\s+");
    }
    ...
    public boolean isSkipExisting() {
        return Utilities.stringToBoolean(properties.getProperty(SKIP_EXISTING_KEY));
    ...
    public boolean isAllowEmptyMetadata() {
        return Utilities.stringToBoolean(properties.getProperty(ALLOW_EMPTY_METADATA_KEY));
    }
    ...
    public boolean isInputQueryCachable() {
        return Boolean.parseBoolean(properties.getProperty(INPUT_CACHABLE_KEY));
    }
    ...
    public int inputQueryBufferSize() {
        return Integer.parseInt(properties.getProperty(INPUT_QUERY_BUFFER_BYTES_KEY));
    }
    ...
    public int inputResultBufferSize() {
        return Integer.parseInt(properties.getProperty(INPUT_RESULT_BUFFER_SIZE_KEY));
    }
    ...
    public int getInputBatchSize() {
        return Integer.parseInt(properties.getProperty(INPUT_BATCH_SIZE_KEY));
    }
    ...
    public int getMaxRetries() {
        return Integer.parseInt(properties.getProperty(MAX_RETRIES_KEY));
    }
    ...
    public int getOutputBatchSize() {
        return Integer.parseInt(properties.getProperty(OUTPUT_BATCH_SIZE_KEY));
    }
    ...
    public boolean useMultiStmtTxn() {
        return Boolean.parseBoolean(properties.getProperty(USE_MULTI_STMT_TXN_KEY));
    }
    ...
    public boolean encodeOutputUri() {
        return Boolean.parseBoolean(properties.getProperty(ENCODE_OUTPUT_URI_KEY));
    }
    ...
    public boolean useInMemoryUriQueue() {
        String p = properties.getProperty(USE_IN_MEMORY_URI_QUEUE_KEY, USE_IN_MEMORY_URI_QUEUE_DEFAULT);
    ...
    public boolean useQueueFile() {
        return (!useInMemoryUriQueue() && properties.getProperty(INPUT_CONNECTION_STRING_KEY) != null);
    }
    ...
    public String getTmpDir() {
        return properties.getProperty(TMP_DIR_KEY, TMP_DIR_DEFAULT);
    }
    ...
    public String getUriQueueFile() {
        return properties.getProperty(URI_QUEUE_FILE_KEY, URI_QUEUE_FILE_DEFAULT);
    }
    ...
    public boolean keepUriQueueFile() {
        String p = properties.getProperty(KEEP_URI_QUEUE_FILE_KEY, KEEP_URI_QUEUE_FILE_DEFAULT);
        return Boolean.parseBoolean(p);
    }
    ...
    public boolean doPrintCurrRate() {
        String p = properties.getProperty(PRINT_CURRENT_RATE_KEY, PRINT_CURRENT_RATE_DEFAULT);
        return Boolean.parseBoolean(p);
    }
    ...
    public boolean useInForestEval() {
        String p = properties.getProperty(USE_IN_FOREST_EVAL_KEY, USE_IN_FOREST_EVAL_DEFAULT);
        return Boolean.parseBoolean(p);
    }
    ...
    private void getPermissionRole(String propertyKey, ContentCapability capability) {
        ...
        if (rolesString != null) {
            ...
            for (String roleName : roleNames) {
                permissionRoles.add(new ContentPermission(capability, roleName));
            }
        }
    }
